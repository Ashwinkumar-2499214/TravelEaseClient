  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h5 className="mb-0">{agentMode ? 'Bookings (Agent View)' : 'My Bookings'}</h5>
        <button className="btn btn-primary btn-sm" onClick={openCreate}><i className="fa-solid fa-plus me-1" />New Booking</button>
      </div>
      <div className="table-responsive">
        <table className="table table-hover table-bordered align-middle">
          <thead className="table-dark">
            <tr><th>Reference</th><th>Start</th><th>End</th><th>Status</th><th>Change Status</th><th>Actions</th></tr>
          </thead>
          <tbody>
            {bookings.map(b => (
              <React.Fragment key={b.id}>
                <tr>
                  <td>
                    <button className="btn btn-link btn-sm p-0 me-2 text-decoration-none" onClick={() => setExpandedId(expandedId === b.id ? null : b.id)}>
                  ...
                  <td>
                    <select className="form-select form-select-sm" style={{ width: 130 }} value={b.status}
                      onChange={e => handleStatus(b.id, e.target.value)}>
                      {STATUSES.map(s => <option key={s} value={s}>{s}</option>)}
                    </select>
                  </td>
                  <td>
                    <button className="btn btn-sm btn-outline-primary me-1" onClick={() => openEdit(b)}><i className="fa-solid fa-pen" /></button>
                    <button className="btn btn-sm btn-outline-danger" onClick={() => handleDelete(b.id)}><i className="fa-solid fa-trash" /></button>
                  </td>
                </tr>
                {expandedId === b.id && <ReservationsRow bookingId={b.id} />}
              </React.Fragment>
            ))}
            {bookings.length === 0 && <tr><td colSpan={6} className="text-center text-muted">No bookings found.</td></tr>}
          </tbody>
        </table>
      </div>
  ...
  return (
    <tr className="table-light">
      <td colSpan={6} className="ps-5">
        <strong className="d-block mb-2">Reservations</strong>
        {loading ? <div className="spinner-border spinner-border-sm" /> : (
          <>
            <table className="table table-sm table-bordered mb-2">
              <thead className="table-secondary"><tr><th>ID</th><th>Seat</th><th>Status</th><th>Change Status</th></tr></thead>
              <tbody>
                {reservations.map(r => (
                  <tr key={r.id}>
                    <td>{r.id}</td>
                    ...
                    <td>
                      <select className="form-select form-select-sm" style={{ width: 130 }} value={r.status}
                ...
                {reservations.length === 0 && <tr><td colSpan={4} className="text-muted text-center">No reservations.</td></tr>}
              </tbody>
            </table>
            {!showAdd ? (
              <button className="btn btn-sm btn-outline-success" onClick={() => setShowAdd(true)}><i className="fa-solid fa-plus me-1" />Add Reservation</button>
            ) : (
              <form onSubmit={addRes} className="d-flex gap-2 flex-wrap">
                <input className="form-control form-control-sm" style={{ width: 160 }} placeholder="Seat Number" value={form.seatNumber} onChange={e => setForm(p => ({ ...p, seatNumber: e.target.value }))} />
                <input className="form-control form-control-sm" style={{ width: 200 }} placeholder="Special Requests" value={form.specialRequests} onChange={e => setForm(p => ({ ...p, specialRequests: e.target.value }))} />
